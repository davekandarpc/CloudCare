...
const UserProfilePage = () => {
  ...
  let sysUserId = url.searchParams.get("sysUserId");

  useEffect(() => {


    let user_type = url.searchParams.get("userType");
    setTimeout(() => {
      if (user_type !== null) {
  ...
  const updatePatient = async () => {
    ...
    if (document.getElementById("postalCode") !== null) {
      postalCodeField = document.getElementById("postalCode").value;
    }
    let provinceField = '';
    if (document.getElementById("province") !== null) {
      provinceField = document.getElementById("province").value;
    }
    let countryField = '';
    if (document.getElementById("country") !== null) {
      countryField = document.getElementById("country").value;
    }
      ...
      <Grid container spacing={2}>
        <Grid item sm={12} xs={12}>
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography className={classes.activeBreadcrumb}>User Profile</Typography>
          </Breadcrumbs>
        </Grid>
        <Grid item sm={3} xs={12} className={classes.menuSide}>
          <div className={classes.title}>{t("userProfile")}</div>
