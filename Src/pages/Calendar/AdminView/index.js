...
import AdminSelectDoctorCol from '../../../components/CalendarComponents/AdminSelectDoctorCol';
...
const AdminView = ({ propsSelectedDoctor, userType, propsType }) => {
    ...
    useEffect(() => {
        if (propsSelectedDoctor !== null) {
            if (scheduleView) {
                getScheduleData();
            } else {
                let serviceProviderId = propsSelectedDoctor.serviceProviderId;
                let sysUserId = propsSelectedDoctor.sysUserId;
                getAvailableSlots(sysUserId, serviceProviderId)
            }
        }
    }, [propsSelectedDoctor])
    useEffect(() => {
        if (propsSelectedDoctor !== null && propsType === 'DELETE_APPOINTMENT_SUCCESS' || propsType === 'CREATE_APPOINTMENT_SUCCESS') {
            let serviceProviderId = propsSelectedDoctor.serviceProviderId;
            let sysUserId = propsSelectedDoctor.sysUserId;
            getAvailableSlots(sysUserId, serviceProviderId)
        }
    ...
    const getAvailableSlots = async (sysUserId, serviceProviderId) => {
        ...
        if (response.ok) {
            ...
            getAppointments(serviceProviderId, data)
        }
    }

    const getAppointments = async (serviceProviderId, availableSlots) => {
        let URL = `appointment/serviceProvider/${serviceProviderId}?tenant=${TENANT_ID}`
            ...
            if (data) {
                ...
                for (let i = 0; i < combinedData.length; i++) {
                    if (combinedData[i].appointmentId !== null) {
                        ...
                        if (combinedData[i].appointmentStatus === 'CONSULTATION_COMPLETED') {
                            title = t('completed')
                            color = '#ff0042';
                        }
                        let eventObj = {
                        ...
                        let eventObj = {
                            ...
                            availabilitySlotIds: combinedData[i].serviceProviderAvailabilitySlotId,
    ...
    const getScheduleData = async () => {
        setLoading(true);
        if (propsSelectedDoctor !== null) {
            const serviceProviderId = propsSelectedDoctor.serviceProviderId;
            var a = new Date().getTimezoneOffset();
            let currentYear = new Date().getFullYear();
            let fromDate = `${currentYear}-01-01`;
            let toDate = `${currentYear}-12-31`;
            var timrZoneOffset = -Math.round(a / 60) + ":" + -(a % 60);
            let URL = `serviceProvider/${serviceProviderId}/schedule?tenant=${TENANT_ID}&timeZoneId=GMT%2B${timrZoneOffset}&fromDateStr=${fromDate}&toDateStr=${toDate}`;
            let response = await getRequest(URL, true);
            if (response.ok) {
                const data = await response.json();
    ...
    const addAvailability = async (closeModalFunction) => {
        ...
        if (hasError) {
            ...
            const sysUserId = propsSelectedDoctor.sysUserId;
    ...
    return (
        <Grid container spacing={2}>
            <Grid item xs={12} sm={!scheduleView ? 8 : 12}>
                <div className={classes.changeToScheduleView}>
                    <div style={{ width: 'fit-content' }}>
                        <AdminSelectDoctorCol />
                    </div>
            ...
            {
                !scheduleView ?
                    <Grid item xs={12} sm={4} style={{ paddingTop: '11%' }}>
